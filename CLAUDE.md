...
- **`src/processor.py`** — `ContentProcessor`: HTML cleaning with BeautifulSoup, LLM summarization through the writer role (Opus 4.6). Auto mode uses 24,000 char threshold to decide summarize vs verbatim. Converts HTML tables to prose (small tables inline, large tables via the LLM) before text extraction.
...
|----------|----------|---------|
| `OPENROUTER_API_KEY` | Yes | OpenRouter: the writer (Opus 4.6), checker (Sonnet 5) and normaliser (Gemini 3 Flash) roles |
| `OPENAI_API_KEY` | No | OpenAI direct: GPT-5.6 Sol writes email bullets and backs up the writer when Claude is unavailable |
| `ANTHROPIC_API_KEY` | No | Anthropic direct: backup route for the Claude roles. All three LLM keys are the ARROW_* accounts |
...
GitHub Actions workflow (`.github/workflows/update-feed.yml`) is scheduled daily at 03:43 UTC; the desktop cron backstop dispatches at 05:30 Europe/London. It runs the pipeline, commits results, then deploys `output/` to GitHub Pages and, once the live `feed.xml` is byte-identical to the committed one (the Pages CDN caches for 10 minutes), sends a WebSub publish ping to the hub the feed declares. Manual dispatch supports `entry_url`, `force_verbatim`, `inject_url`, `inject_mode`, and `resend_report` inputs. `llm_smoke_test=true` runs only short synthetic LLM checks with the real CI keys: no email, audio, state changes, or publishing. Run it locally with `bwsrun uv run python -m scripts.check_llm`. A `concurrency` group (`feedcast-pipeline`) prevents race conditions between scheduled and injected runs. The commit step uses `git pull --rebase` to handle sequential runs cleanly.
