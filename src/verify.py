...
async def check(script: str, source: str, client=None) -> tuple[int, list[dict]]:
    ...
    for i, reasoning in enumerate(CHECK_ATTEMPTS):
        try:
                ...
                # exists for: retry the same model with reasoning off rather
                # than hand the check to a sibling model and report a fallback.
                # The last attempt may fall back like any other call.
